...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      renderer = new THREE.WebGLRenderer({ antialias: false, alpha: true, powerPreference: effectiveQuality === 'low' ? 'low-power' : 'high-performance', precision: settings.precision, stencil: false, depth: false });
    ...
    const animate = (currentTime: number) => {
      ...
      if (deltaTime >= frameTime) {
        ...
        materialRef.current.uniforms.uRotCos.value = Math.cos(timeRef.current * 0.3);
        materialRef.current.uniforms.uRotSin.value = Math.sin(timeRef.current * 0.3);
    ...
    return () => {
      ...
      if (rendererRef.current) { rendererRef.current.dispose(); rendererRef.current.forceContextLoss(); if (container.contains(rendererRef.current.domElement)) container.removeChild(rendererRef.current.domElement); }
  ...
  useEffect(() => { if (!materialRef.current) return; const c = new THREE.Color(topColor); materialRef.current.uniforms.uTopColor.value = new THREE.Vector3(c.r, c.g, c.b); }, [topColor]);
  useEffect(() => { if (!materialRef.current) return; const c = new THREE.Color(bottomColor); materialRef.current.uniforms.uBottomColor.value = new THREE.Vector3(c.r, c.g, c.b); }, [bottomColor]);
  useEffect(() => { if (!materialRef.current) return; materialRef.current.uniforms.uIntensity.value = intensity; }, [intensity]);
  useEffect(() => { if (!materialRef.current) return; materialRef.current.uniforms.uInteractive.value = interactive; }, [interactive]);
  ...
  useEffect(() => { if (!materialRef.current) return; const r = (pillarRotation * Math.PI) / 180; materialRef.current.uniforms.uPillarRotCos.value = Math.cos(r); materialRef.current.uniforms.uPillarRotSin.value = Math.sin(r); }, [pillarRotation]);

  if (!webGLSupported) return <div className={`w-full h-full absolute top-0 left-0 ${className}`} style={{ mixBlendMode }} />;
