...
export default function GooeyNav({
  ...
  const makeParticles = (element: HTMLElement) => {
    const d = particleDistances; const r = particleR;
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        particle.classList.add('particle'); point.classList.add('point');
  ...
  const updateEffectPosition = (element: HTMLElement) => {
    ...
    Object.assign(filterRef.current.style, styles); Object.assign(textRef.current.style, styles);
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    ...
    if (onNavigate) onNavigate(items[index].href);
  };

  useEffect(() => {
    if (!navRef.current || !containerRef.current) return;
    const activeLi = navRef.current.querySelectorAll('li')[activeIndex];
    if (activeLi) { updateEffectPosition(activeLi as HTMLElement); textRef.current?.classList.add('active'); }
    const resizeObserver = new ResizeObserver(() => { const li = navRef.current?.querySelectorAll('li')[activeIndex]; if (li) updateEffectPosition(li as HTMLElement); });
